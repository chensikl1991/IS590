...
def read_one_HURDAT2_storm(file, storm_id=None) -> dict:
    ...
    >>> f = open('data/hurdat2-1851-2017-050118.txt', 'r')
...
def get_max_wind_speed(storm: dict) -> int:
    ...
    >>> f.close()
    """
    highest = 0  # start at zero. Contains highest value found so far.
...
def main():
    """Script main, to be executed as a demonstration of functions here."""
    ...
    print('With the given coordinates, AL051952 moved {:.2f} nm at initial heading of {:.2f} deg.'.format(
            distance, bearing))
    print('\n')

    with open('data/hurdat2-1851-2017-050118.txt', 'r') as f:
        ...
        print('Searching for storms that exceeded {} knots...'.format(wind_limit))
        while True:
            s = read_one_HURDAT2_storm(f)
            if s is None:
                break      # hit end of file
            max_wind = get_max_wind_speed(s)
            if max_wind >= wind_limit:
                print(s['id'], end=' ')
                print('highest wind:', get_max_wind_speed(s), '\n')


"""
It's a Python best practice to use the special conditional below for at least 3 reasons:
    1. It allows Doctests run properly without interference, 
    2. It makes Doctest coverage analysis accurate (not inflated by executing non-test code),
    3. It allows importing this script as a module to use its functions and/or classes in another program.
"""
