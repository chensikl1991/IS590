...
class ChessPiece:
    ...
    @staticmethod
    def clear_board():
        while len(ChessPiece.__all_pieces):
            piece = ChessPiece.__all_pieces.pop(0)
            del piece
    ...
    def __repr__(self) -> str:
        """This special function gets called when there's a need to display
        the object itself, such as in error messages, or in the IPython console.
        By convention and when practical, this should be a valid Python expression
        that could be sent to eval() to create an equivalent instance.

        Also, if the __str__() method is not defined, this will get used in its
        place.

        :return: a string representation of the object.
        """
        return "{}('{}', '{}')".format(
                self.__class__.__name__, self.color, self.location)
...
class Pawn(ChessPiece):
    ...
    def promote(self, new_piece_class: callable):
        location = self.location
        color = self.color
        del self

        new_piece_class(color, location)
...
if __name__ == '__main__':
    ...
    ChessPiece.list_pieces()  # show all the pieces on the board.
